...
const entry = {};
append(entry, './src', '');
fs.readdirSync(path.resolve('./src')).forEach((dirname) => append(entry, `./src/${dirname}`, dirname));
      ...
      patterns: [
        {
          ...
          transform(content) {
            ...
            fs.readdirSync('./src').forEach((dirname) => buffer['content_scripts'][0]['js'] = push(buffer['content_scripts'][0]['js'], `./src/${dirname}`, dirname));
            return Buffer.from(
              JSON.stringify({
